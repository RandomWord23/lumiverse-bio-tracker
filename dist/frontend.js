export function setup(ctx) {
  ...
  `;
    ...
    // ─── Settings System ───────────────────────────────────────
    const defaultToastSettings = { digestionTicks: true, climaxEvents: true, clothingDamage: true, nutrientAbsorption: false, digestionSkips: false, sheetSync: true, rollbackEvents: true, rollbackWarnings: true, struggleEvents: true, vomitEvents: true, errors: true, chatWarnings: false };
    const defaultEngineToggles = { digestionEngine: true, clothingStress: true, nutrientAbsorption: true, arousalClimax: true, struggleEngine: true };
    const defaultUiSettings = { btnOpacity: 0.4, panelWidth: 350, autoOpen: false };
    ...
    function saveSettings(s) { localStorage.setItem('bio-tracker-settings', JSON.stringify(s)); }
    function sendSettingsToBackend(s) { ctx.sendToBackend({ type: 'SETTINGS_UPDATED', toastSettings: s.toast, engineToggles: s.engine }); }
    function applyUiSettings(ui) {
        const pe = document.getElementById('bio-tracker-panel');
        if (pe)
            pe.style.width = ui.panelWidth + 'px';
        if (floatingBtn) {
            floatingBtn.style.opacity = String(ui.btnOpacity);
            clearTimeout(fadeTimeout);
            fadeTimeout = setTimeout(() => { floatingBtn.style.opacity = String(ui.btnOpacity); }, 3000);
        }
    }
    let currentSettings = loadSettings();
    function buildToggleRow(def, section) {
    ...
    if (toastContainer)
        toastCategoryDefs.forEach((d) => toastContainer.appendChild(buildToggleRow(d, 'toast')));
    const engineContainer = document.getElementById('bt-engine-settings');
    if (engineContainer)
        engineToggleDefs.forEach((d) => engineContainer.appendChild(buildToggleRow(d, 'engine')));
    const opacitySlider = document.getElementById('bt-set-opacity');
    const opacityVal = document.getElementById('bt-set-opacity-val');
    if (opacitySlider) {
        opacitySlider.value = String(currentSettings.ui.btnOpacity);
        if (opacityVal)
            opacityVal.textContent = String(currentSettings.ui.btnOpacity);
    ...
    document.getElementById('bt-set-reset-all')?.addEventListener('click', () => {
        ...
        if (opacitySlider) {
            ...
            if (opacityVal)
                opacityVal.textContent = '0.4';
        }
        if (widthSlider) {
            widthSlider.value = '350';
            if (widthVal)
                widthVal.textContent = '350px';
        }
        if (autoOpenSwitch)
            autoOpenSwitch.classList.remove('on');
        applyUiSettings(currentSettings.ui);
    });
    sendSettingsToBackend(currentSettings);
    // ─── Floating Button ───────────────────────────────────────
    ...
    function updateAttrModDisplay(attrKey) {
        const input = document.getElementById('bt-attr-' + attrKey.toLowerCase());
    ...
    // ─── Currency toggle ───────────────────────────────────────
    const currencyType = document.getElementById('bt-currency-type');
    currencyType?.addEventListener('change', () => {
        ...
        if (modern)
            modern.style.display = currencyType.value === 'modern' ? 'block' : 'none';
        if (fantasy)
            fantasy.style.display = currencyType.value === 'fantasy' ? 'flex' : 'none';
    });
    // ─── Capacity calculator ───────────────────────────────────
    function updateCapacities() {
        const heightEl = document.getElementById('bt-height');
        const weightEl = document.getElementById('bt-weight');
        const multEl = document.getElementById('bt-cap-mult');
        if (!heightEl || !weightEl || !multEl)
            return;
        ...
        document.querySelectorAll('.stomach-vol').forEach((el) => {
            stomTotal += parseFloat(el.value) || 0;
        });
        ...
        document.querySelectorAll('.bowel-vol').forEach((el) => {
            bowelTotal += parseFloat(el.value) || 0;
        });
        ...
        document.querySelectorAll('#stomach-container .vital-slot').forEach((el) => {
            const type = el.querySelector('.v-type')?.value;
            if (type !== 'Prey')
                return;
            preyCount++;
            const vol = parseFloat(el.querySelector('.v-vol')?.value || '0') || 0;
            totalPreyVolume += vol;
            const will = el.querySelector('.v-willingness')?.value || 'Reluctant';
            if (will === 'Fighting')
                fightingCount++;
            else if (will === 'Willing')
                willingCount++;
            else
                reluctantCount++;
        });
        ...
        if (riskEl && detailEl) {
            if (preyCount === 0) {
                ...
                let riskLabel;
                let riskColor;
                if (riskScore >= 80 || indigestion >= 90) {
                    riskLabel = 'CRITICAL — Vomit imminent';
                    riskColor = '#ff0000';
                }
                else if (riskScore >= 50 || indigestion >= 75) {
                    riskLabel = 'High — Vomit likely soon';
                    riskColor = '#ff4444';
                }
                else if (riskScore >= 25 || indigestion >= 50) {
                    riskLabel = 'Moderate — Building pressure';
                    riskColor = '#ff9800';
                }
                else if (riskScore >= 10 || indigestion >= 25) {
                    riskLabel = 'Low — Some unrest';
                    riskColor = '#ffeb3b';
                }
                else {
                    riskLabel = 'Minimal — Calm';
                    riskColor = '#4CAF50';
                }
                ...
                const parts = [];
                if (fightingCount > 0)
                    parts.push(`${fightingCount} fighting`);
                if (reluctantCount > 0)
                    parts.push(`${reluctantCount} reluctant`);
                if (willingCount > 0)
                    parts.push(`${willingCount} willing`);
    ...
    `;
        const arousalInput = document.getElementById('bt-arousal-slider');
    ...
    function setArousalSlider(v) {
        const input = document.getElementById('bt-arousal-slider');
    ...
    function setClimaxSlider(v) {
        const input = document.getElementById('bt-climax-slider');
    ...
    function updateEnergyDisplay() {
        const energyInput = document.getElementById('bt-energy');
        if (!energyInput)
            return;
    ...
    function updateCurrentPenisSize(arousalVal) {
        const maxL = parseFloat(document.getElementById('bt-penis-len')?.value) || 0;
        const maxG = parseFloat(document.getElementById('bt-penis-girth')?.value) || 0;
    ...
    panel.addEventListener('input', (e) => {
        ...
        if (target.classList.contains('item-dig-input')) {
            ...
            const statusSpan = slot?.querySelector('.item-status');
    ...
    `;
        const typeSelect = div.querySelector('.v-type');
        const gearArea = div.querySelector('.v-gear');
        const appearanceArea = div.querySelector('.v-appearance');
        const statusSpan = div.querySelector('.item-status');
        const willingnessRow = div.querySelector('.v-prey-willingness');
    ...
    function collectBuffsFromItem(el) {
        const entries = [];
        el.querySelectorAll('.bt-buff-entry').forEach((buffEl) => {
            const stat = buffEl.querySelector('.bt-buff-stat')?.value;
            const pct = buffEl.querySelector('.bt-buff-pct')?.value.trim();
            if (stat && pct) {
    ...
    function addFlagButtons() {
        ...
        fields.forEach((field) => {
            ...
            if (container.querySelector('.bt-flag-btn'))
                return;
    ...
    function buildCurrentXml() {
        ...
        for (const ak of attrKeys) {
            const attrInput = document.getElementById('bt-attr-' + ak.toLowerCase());
        ...
        document.querySelectorAll('.bt-cloth-slot').forEach((el) => {
            ...
            if (val !== '') {
                const flexEl = input.previousElementSibling?.querySelector('.bt-cloth-flex');
        ...
        document.querySelectorAll('.dyn-inv').forEach((el) => {
            const qty = el.querySelector('.d-qty')?.value.trim() || '1';
            const name = el.querySelector('.d-name')?.value.trim();
            if (name)
                xml += `    <Item qty="${qty}">${name}</Item>\n`;
        });
        xml += `  </Backpack>\n\n  <SkillsAndTraits>\n`;
        document.querySelectorAll('.dyn-skill').forEach((el) => {
            const name = el.querySelector('.d-name')?.value.trim();
            const lvl = el.querySelector('.d-lvl')?.value.trim() || '1';
            const desc = el.querySelector('.d-desc')?.value.trim();
            const buffsStr = collectBuffsFromItem(el);
            let attrs = `name="${name}" level="${lvl}"`;
            if (buffsStr)
                attrs += ` buffs="${buffsStr}"`;
            if (name)
                xml += `    <Skill ${attrs}>${desc}</Skill>\n`;
        });
        ...
        const suppressing = document.getElementById('bt-suppressing-toggle')?.checked ? 'true' : 'false';
        xml += `    <Stomach current="${stomFill}" max="${stomMax}" suppressing="${suppressing}">\n`;
        document.querySelectorAll('#stomach-container .vital-slot').forEach((el) => {
            const name = el.querySelector('.v-name')?.value.trim() || 'Unknown';
            const vol = el.querySelector('.v-vol')?.value.trim() || '0';
            const dig = el.querySelector('.v-dig')?.value.trim() || '0';
            const type = el.querySelector('.v-type')?.value || 'Food';
            const flavor = el.querySelector('.v-flavor')?.value.trim();
            const gear = el.querySelector('.v-gear')?.value.trim();
            const appearance = el.querySelector('.v-appearance')?.value.trim();
            let itemAttrs = `type="${type}" name="${name}" volume_L="${vol}" digestion="${dig}%"`;
            if (type === 'Prey') {
                const willingness = el.querySelector('.v-willingness')?.value || 'Reluctant';
                const staminaText = el.querySelector('.v-stamina-val')?.textContent || '100%';
        ...
        document.querySelectorAll('#bowel-container .vital-slot').forEach((el) => {
            const name = el.querySelector('.v-name')?.value.trim() || 'Waste';
            const vol = el.querySelector('.v-vol')?.value.trim() || '0';
    ...
    document.getElementById('bt-populate-btn')?.addEventListener('click', () => {
        ...
        const btn = document.getElementById('bt-populate-btn');
        if (flagged.length === 0) {
            if (btn) {
    ...
    const breastInput = document.getElementById('bt-breast-ml');
    const breastCup = document.getElementById('bt-breast-cup');
    ...
    function applyColorEffect(inputId) {
    ...
    const unsubTag = ctx.messages.registerTagInterceptor({ tagName: 'sheet_update', removeFromMessage: true }, () => { });
    // ─── Backend message handler ───────────────────────────────
    ctx.onBackendMessage((msg) => {
        ...
        if (msg.type === 'POPULATE_DONE') {
            ...
            if (msg.success) {
                panel.querySelectorAll('.bt-flag-btn[data-flagged="true"]').forEach((b) => {
                    ...
                    const input = panel.querySelector(`[data-id="${fieldId}"], [data-slot="${fieldId}"]`);
                    if (input) {
    ...
    function getSwipeText(swipe) {
        ...
        if (swipe && typeof swipe === 'object') {
            ...
            if (Array.isArray(swipe.content)) {
                return swipe.content.filter((p) => p.type === 'text').map((p) => p.text).join('\n');
            }
            if (typeof swipe.text === 'string')
                return swipe.text;
        }
        return '';
    }
    function extractSheetUpdateFromText(text) {
        if (!text)
            return null;
    ...
    ctx.events.on('MESSAGE_SWIPED', (payload) => {
    ...
    // ─── XML to form parser ────────────────────────────────────
    function populateFormFromXml(xml) {
        ...
        if (attrBlock) {
            ...
            for (const ak of attrKeys) {
                ...
                const input = document.getElementById('bt-attr-' + ak.toLowerCase());
                if (node && input)
                    input.value = node.textContent || '10';
                if (ak)
        ...
        if (currencySystem) {
            const currSelect = document.getElementById('bt-currency-type');
            if (currSelect) {
        ...
        doc.querySelectorAll('Equip').forEach((equipNode) => {
            ...
            if (!slot)
                return;
            const input = document.querySelector(`.bt-cloth-slot[data-slot="${slot}"]`);
            if (input) {
                input.value = value;
                const flexSelect = input.previousElementSibling?.querySelector('.bt-cloth-flex');
                if (flexSelect)
                    flexSelect.value = elasticity;
                let labelEl = input.previousElementSibling;
                if (labelEl && labelEl.classList.contains('flex-row')) {
                    labelEl = labelEl.querySelector('.slot-label');
                }
                if (labelEl) {
                    let badge = labelEl.querySelector('.cloth-badge');
                    if (!badge) {
        ...
        doc.querySelectorAll('Backpack > Item').forEach((itemNode) => {
            ...
            div.querySelector('.d-qty').value = qty;
            div.querySelector('.d-name').value = name;
        });
        doc.querySelectorAll('Skill').forEach((skillNode) => {
            ...
            div.querySelector('.d-name').value = skillNode.getAttribute('name') || '';
            div.querySelector('.d-lvl').value = skillNode.getAttribute('level') || '1';
            div.querySelector('.d-desc').value = skillNode.textContent || '';
            ...
            if (buffsAttr) {
                ...
                buffsAttr.split(';').forEach(pair => {
                    ...
                    if (stat && pct && container) {
                        ...
                        entry.querySelector('.bt-buff-stat').value = stat.trim();
                        entry.querySelector('.bt-buff-pct').value = pct.trim();
        ...
        doc.querySelectorAll('Trait').forEach((traitNode) => {
            ...
            div.querySelector('.d-name').value = traitNode.getAttribute('name') || '';
            div.querySelector('.d-desc').value = traitNode.textContent || '';
        ...
        if (stomachNode) {
            ...
            if (indVal)
                indVal.textContent = `${Math.round(indigestion)}%`;
            const suppressing = stomachNode.getAttribute('suppressing') === 'true';
            const suppressToggle = document.getElementById('bt-suppressing-toggle');
            const suppressLabel = document.getElementById('bt-suppressing-label');
            if (suppressToggle)
                suppressToggle.checked = suppressing;
            if (suppressLabel)
                suppressLabel.textContent = suppressing ? 'Active' : 'Passive';
            ...
            if (fatigueInfo) {
                if (stomachFatigue > 20)
                    fatigueInfo.textContent = `Fatigue: High (${Math.round(stomachFatigue)})`;
                else if (stomachFatigue > 10)
                    fatigueInfo.textContent = `Fatigue: Moderate (${Math.round(stomachFatigue)})`;
                else if (stomachFatigue > 0)
                    fatigueInfo.textContent = `Fatigue: Low (${Math.round(stomachFatigue)})`;
                else
                    fatigueInfo.textContent = '';
            }
        }
        doc.querySelectorAll('Stomach > Item').forEach((itemNode) => {
            ...
            div.querySelector('.v-name').value = getAttr(itemNode, 'name');
            div.querySelector('.v-vol').value = getAttr(itemNode, 'volume_L');
            div.querySelector('.v-dig').value = (getAttr(itemNode, 'digestion') || '').replace('%', '');
            const type = getAttr(itemNode, 'type') || 'Food';
            const typeSelect = div.querySelector('.v-type');
            typeSelect.value = type;
            typeSelect.dispatchEvent(new Event('change'));
            const appearanceNode = itemNode.querySelector('Appearance');
            div.querySelector('.v-appearance').value = appearanceNode?.textContent || '';
            const descNode = itemNode.querySelector('Description');
            div.querySelector('.v-flavor').value = descNode?.textContent || '';
            if (type === 'Prey') {
                const gearNode = itemNode.querySelector('BoundGear');
                div.querySelector('.v-gear').value = gearNode?.textContent || '';
                const willingness = getAttr(itemNode, 'willingness') || 'Reluctant';
                const willingnessSelect = div.querySelector('.v-willingness');
                if (willingnessSelect)
                    willingnessSelect.value = willingness;
                const stamina = parseFloat(getAttr(itemNode, 'stamina') || '100') || 100;
                const staminaBar = div.querySelector('.v-stamina-bar');
                const staminaVal = div.querySelector('.v-stamina-val');
                if (staminaBar) {
                ...
                if (staminaVal)
                    staminaVal.textContent = `${Math.round(stamina)}%`;
            }
            const digInput = div.querySelector('.item-dig-input');
            if (digInput)
                digInput.dispatchEvent(new Event('input', { bubbles: true }));
        });
        doc.querySelectorAll('Remains').forEach((remainsNode) => {
            ...
            div.querySelector('.v-name').value = remainsNode.textContent || '';
            div.querySelector('.v-vol').value = getAttr(remainsNode, 'volume_L');
        });
        updateCapacities();
    }
    // ─── Cleanup ───────────────────────────────────────────────
