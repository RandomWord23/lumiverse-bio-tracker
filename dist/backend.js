...
let pendingGenerationType = 'normal';
...
function maybeToast(category, type, message) {
    if (toastSettings[category] === false)
        return;
    spindle.toast[type](message);
}
function sheetPath(chatId) {
    return `sheets/${chatId}.xml`;
}
function snapshotsPath(chatId) {
    return `snapshots/${chatId}.json`;
}
async function loadChatSheet(chatId) {
    try {
...
async function saveChatSheet(chatId, xml) {
    sheets.set(chatId, xml);
    await spindle.storage.write(sheetPath(chatId), xml);
}
async function loadChatSnapshots(chatId) {
    try {
...
async function saveChatSnapshots(chatId) {
...
function extractSheetUpdate(content) {
    let text = '';
    if (typeof content === 'string') {
        text = content;
    }
...
function collectBuffs(xml) {
    const buffs = {};
    const parseBuffsAttr = (attrs) => {
    ...
    let m;
    while ((m = skillRegex.exec(xml)) !== null)
        parseBuffsAttr(m[1]);
    const traitRegex = /<Trait\s+([^>]*?)>/gi;
    while ((m = traitRegex.exec(xml)) !== null)
        parseBuffsAttr(m[1]);
    return buffs;
}
// ─── Modifier Pipeline ──────────────────────────────────────
...
function applyModifierCap(modifiers) {
    const capped = {};
...
function getStat(xml, tag) {
    const match = xml.match(new RegExp(`<${tag}>(.*?)<\\/${tag}>`, 'i'));
    return match ? parseFloat(match[1]) || 0 : 0;
}
function setStat(xml, tag, value) {
    ...
    if (xml.includes('</BaseStats>')) {
        return xml.replace(/<\/BaseStats>/i, `    <${tag}>${value.toFixed(2)}</${tag}>\n  </BaseStats>`);
    }
    return xml;
}
...
function deriveCondition(stress, elasticity, lockedCondition) {
    if (elasticity === 'magic')
        return 'intact';
...
function processClothingStress(xml, oldXml, clothingStressMult = 0) {
    const damageEvents = [];
    const getMode = (x) => {
    ...
    xml = xml.replace(/<Equip\s+([^>]*?)>([\s\S]*?)<\/Equip>/gi, (match, attrs, inner) => {
        ...
        let stress = Math.max(parseFloat(getAttrFromString(attrs, 'stress')) || 0, oldEq?.stress || 0);
    ...
    xml = xml.replace(/<Equip\s+([^>]+?)\s*\/>/gi, (match, attrs) => {
        const slot = getAttrFromString(attrs, 'slot') || '';
        const elasticity = getAttrFromString(attrs, 'elasticity') || 'standard';
        if (elasticity === 'magic') {
            let cleanAttrs = attrs
...
function digestItemsInContent(content, ctx) {
    let totalDigestedVol = 0;
    let wasteCount = 0;
    let accumulatedWasteVol = 0;
    const newRemains = [];
    let itemCount = 0;
    const digestItem = (attrs, inner, isSelfClosing) => {
        itemCount++;
    ...
    content = content.replace(/<Item\s+([^>]*[^>\/])\s*>([\s\S]*?)<\/Item>/gi, (match, attrs, inner) => digestItem(attrs, inner, false));
    content = content.replace(/<Item\s+([^>]+?)\s*\/>/gi, (match, attrs) => digestItem(attrs, null, true));
    return {
        content,
        totalDigestedVol,
        wasteCount,
...
function processStruggle(xml, oldXml, elapsed, stomachResistanceMult = 0, energyDrainMult = 0) {
    ...
    let indigestion = Math.max(parseFloat(getAttrFromString(stomAttrs, 'indigestion')) || oldIndigestion, oldIndigestion);
    const suppressing = getAttrFromString(stomAttrs, 'suppressing') === 'true';
    const oldStomachFatigue = parseFloat(getAttrFromString(oldStomAttrs, 'stomachFatigue')) || 0;
    let stomachFatigue = Math.max(parseFloat(getAttrFromString(stomAttrs, 'stomachFatigue')) || oldStomachFatigue, oldStomachFatigue);
    ...
    const updateStomachTag = (content) => {
        return xml.replace(/<Stomach([^>]*)>[\s\S]*?<\/Stomach>/i, (_match, attrs) => {
            let newAttrs = attrs
    ...
    const preyData = [];
    const collectPrey = (attrs, inner, isSelfClosing) => {
        if ((getAttrFromString(attrs, 'type') || 'Food') !== 'Prey')
            return;
        const name = getAttrFromString(attrs, 'name');
        ...
        let stamina = Math.max(parseFloat(getAttrFromString(attrs, 'stamina')) || oldStamina, oldStamina);
        // Consciousness factor from digestion %
        let consciousnessFactor;
        if (digPct < 50)
            consciousnessFactor = 1.0;
        else if (digPct < 70)
            consciousnessFactor = 0.5;
        else if (digPct < 85)
            consciousnessFactor = 0.1;
        else
            consciousnessFactor = 0.0;
        // Size factor
        const sizeFactor = stomachMaxCapacity > 0 ? Math.min(2.0, vol / stomachMaxCapacity) : 1.0;
        // Willingness factor
        let willingnessFactor;
        if (willingness === 'willing')
            willingnessFactor = 0;
        else if (willingness === 'fighting')
            willingnessFactor = 1.0;
        else
        ...
        if (willingness === 'fighting') {
            ...
            if (stamina <= 0) {
                ...
                willingnessFactor = 0.25;
                struggleEvents.push(`EXHAUSTED: "${name}" has run out of stamina and can no longer fight — they've gone limp.`);
            }
        }
        else {
        ...
        const personalStruggle = baseIndigestionRate * elapsed * consciousnessFactor * sizeFactor * willingnessFactor;
        preyData.push({
            name,
            volume: vol,
            digestionPct: digPct,
    ...
    stomContent.replace(/<Item\s+([^>]*[^>\/])\s*>([\s\S]*?)<\/Item>/gi, (match, attrs, inner) => {
    ...
    if (energy <= 0) {
        ...
        if (energy <= 20)
            suppressionFactor = Math.min(1.0, suppressionFactor + 0.2);
    }
    else {
    ...
    if (anyFighting) {
        const fightingStruggle = suppressionFactor *
            preyData
    ...
    if (indigestion >= 100) {
        ...
        const remainingPrey = [];
        for (const prey of preyData) {
            const struggleShareFactor = totalStruggle > 0
    ...
    let preyIdx = 0;
    let newStomContent = stomContent.replace(/<Item\s+([^>]*[^>\/])\s*>([\s\S]*?)<\/Item>/gi, (match, attrs, inner) => {
        if ((getAttrFromString(attrs, 'type') || 'Food') !== 'Prey')
            return match;
    ...
    newStomContent = newStomContent.replace(/<Item\s+([^>]+?)\s*\/>/gi, (match, attrs) => {
        if ((getAttrFromString(attrs, 'type') || 'Food') !== 'Prey')
            return match;
        const prey = preyData[preyIdx++];
        if (prey.escaped)
    ...
    xml = setStat(xml, 'Energy', energy);
    spindle.log.info(`Struggle tick: indigestion ${indigestion.toFixed(1)}%, energy ${energy.toFixed(1)}, ` +
        `${numFighting} fighting prey, fatigue ${stomachFatigue.toFixed(1)}`);
    return { xml, struggleEvents };
}
async function runDigestionTick(newXml, oldXml, chatId) {
    try {
        const getTimeHours = (xml) => {
        ...
        if (engineToggles.digestionEngine) {
            ...
            updatedXml = newXml.replace(/<CurrentAcidPct>.*?<\/CurrentAcidPct>/i, `<CurrentAcidPct>${acidLevel.toFixed(2)}</CurrentAcidPct>`);
            if (!updatedXml.includes('<CurrentAcidPct>')) {
                if (updatedXml.includes('</BaseStats>')) {
                    updatedXml = updatedXml.replace(/<\/BaseStats>/i, `    <CurrentAcidPct>${acidLevel.toFixed(2)}</CurrentAcidPct>\n  </BaseStats>`);
                }
                else if (updatedXml.includes('</State>')) {
                    updatedXml = updatedXml.replace(/<\/State>/i, `    <CurrentAcidPct>${acidLevel.toFixed(2)}</CurrentAcidPct>\n  </State>`);
                }
            }
            ...
            if (accumulatedWasteVol > 0) {
                ...
                if (existingWaste) {
                    ...
                    bowContent = bowContent.replace(wasteRegex, `<Remains volume_L="${newVol.toFixed(2)}">Digestive Waste</Remains>`);
                }
                else {
        ...
        if (engineToggles.struggleEngine) {
            ...
            if (struggleResult.struggleEvents.length > 0) {
                await spindle.variables.chat.set(chatId, 'pendingStruggleEvents', JSON.stringify(struggleResult.struggleEvents));
                spindle.log.info(`Struggle events: ${struggleResult.struggleEvents.length} events triggered`);
            }
        } // end struggleEngine
        ...
        if (engineToggles.nutrientAbsorption && totalDigestedVol > 0) {
            ...
            maybeToast('nutrientAbsorption', 'info', `Nutrient absorption: +${heightGrowth.toFixed(2)}cm height, +${weightGrowth.toFixed(2)}kg weight, +${breastGrowth.toFixed(2)}ml breasts`);
            spindle.log.info(`Nutrient absorption: +${heightGrowth.toFixed(2)}cm height, ` +
        ...
        spindle.log.info(`Digestion tick: ${elapsed.toFixed(2)}h elapsed, ` +
...
async function commitUpdate(chatId, messageId, sheetXml, chatIndex) {
...
async function rollbackOnDelete(chatId, messageId) {
    const list = snapshots.get(chatId);
    if (!list) {
...
function buildSheetPrompt(sheetXml) {
...
spindle.onFrontendMessage(async (msg) => {
    if (msg.type === 'SETTINGS_UPDATED') {
        if (msg.toastSettings)
            toastSettings = { ...toastSettings, ...msg.toastSettings };
        if (msg.engineToggles)
            engineToggles = { ...engineToggles, ...msg.engineToggles };
    ...
    if (msg.type === 'POPULATE_FIELDS' && msg.fields) {
        ...
        const existingPopulate = await spindle.variables.chat.get(activeChatId, 'populatePending');
        if (existingPopulate) {
            spindle.sendToFrontend({ type: 'POPULATE_DONE', success: false });
            return;
        }
        const fields = msg.fields;
        await spindle.variables.chat.set(activeChatId, 'populateFields', fields.join(', '));
        // Track that a populate generation is pending so GENERATION_ENDED
        // can notify the frontend when it completes.
        await spindle.variables.chat.set(activeChatId, 'populatePending', 'true');
        try {
            ...
            await spindle.chat.appendMessage(activeChatId, {
                role: 'user',
            ...
            await spindle.variables.chat.delete(activeChatId, 'populateFields');
            await spindle.variables.chat.delete(activeChatId, 'populatePending');
            spindle.sendToFrontend({ type: 'POPULATE_DONE', success: false });
        }
    }
});
spindle.registerInterceptor(async (messages, context) => {
    ...
    const genType = ctx.generationType;
    ...
    const populateFields = await spindle.variables.chat.get(chatId, 'populateFields');
    if (populateFields) {
    ...
    const pendingStruggleEvents = await spindle.variables.chat.get(chatId, 'pendingStruggleEvents');
    if (pendingStruggleEvents) {
        await spindle.variables.chat.delete(chatId, 'pendingStruggleEvents');
        try {
            const events = JSON.parse(pendingStruggleEvents);
            if (events.length > 0) {
    ...
    const cleanedMessages = messages.map((msg) => {
        if (typeof msg.content === 'string') {
            return {
                ...msg,
                content: msg.content.replace(/<sheet_update>[\s\S]*?<\/sheet_update>/gi, ''),
            };
        }
            ...
            return {
                ...
                content: msg.content.map((part) => {
                    if (part.type === 'text' && typeof part.text === 'string') {
                        return {
                            ...part,
                            text: part.text.replace(/<sheet_update>[\s\S]*?<\/sheet_update>/gi, ''),
                        };
                    }
                    return part;
                }),
...
spindle.on('MESSAGE_DELETED', async (payload) => {
    ...
    if (chatId)
        await rollbackOnDelete(chatId, messageId);
});
